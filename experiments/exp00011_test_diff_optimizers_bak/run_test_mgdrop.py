...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 10 09:18:52 2022
...
from train_test_methods import train_mgdrop, validation_mgdrop, train_and_val_mgdrop
...
parser.add_argument('--iters', type=int, default=10000000000, help="number of forward/backward passes before terminating an epoch. Use np.inf if you want to run through all examples in an epoch.")
parser.add_argument('--val_every_n_iters', type=int, default=10000000000, help="get validation accuracy after this many data evaluations. If np.inf then it will evaluate at the end of every epoch as normal.")
...
output_parent = "exp_output_mgdrop"
...
if model_type == "linear":
    ...
    model = NonLinearFC(num_layers, num_features, num_classes, num_hidden_nodes, drop_rate=mgdrop.drop_rates[str(levels)])
...
for epoch in range(epochs):
    train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model = train_and_val_mgdrop(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, mgdrop, levels, base_lr, momentum, weight_decay)
