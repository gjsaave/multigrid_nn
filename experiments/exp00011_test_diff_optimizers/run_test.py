...
parser.add_argument('--padding', type=str, default="same", metavar='N',
                    help='for conv layers. choices are valid, same')
...
if model_type == "linear":
    ...
    model = NonLinearCNN(num_layers, num_features, num_classes, num_hidden_nodes, _drop_rate, num_layers_conv, in_channels, out_channels_init, kernel_size, padding, im_size)
...
for epoch in range(epochs):
    if alg == "baseline":
        train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model = train_and_val(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, num_evals, dataset)

    elif alg == "mgdrop":
         train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model = train_and_val_mgdrop(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, mgdrop, levels, base_lr, momentum, weight_decay, dataset)
