...
from train_test_methods import train_and_val, train_and_val_mgdrop, dropout_coarsening_test
...
output_savepath = os.path.join(savepath, output_parent, "output")
if not os.path.exists(output_savepath):
    os.makedirs(output_savepath)
    
plots_savepath = os.path.join(savepath, output_parent,  "plots")
...
val_accs_all_iters = []
val_accs_all_iters_full = []

for epoch in range(epochs):
    if alg == "baseline":
        train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, val_accs_all_iters_full, model = dropout_coarsening_test(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_accs_all_iters_full, val_every_n_iters, num_evals, dataset, device, num_drops)
...
label_every_n = 2
chosen_vals = np.asarray(val_accs_all_iters)[::label_every_n]
chosen_vals=[a for a in chosen_vals]

chosen_vals_full = np.asarray(val_accs_all_iters_full)[::label_every_n]
chosen_vals_full = list(chosen_vals_full)

add_label(plt.violinplot(chosen_vals, showmeans=True), "dropout network")
#plt.plot([1], [train_accs[-1]], marker="o", markersize=10, markerfacecolor="blue", markeredgecolor="blue", label="train set, no drop")
#plt.plot([2], [val_accs[-1]], marker="o", markersize=10, markerfacecolor="red", markeredgecolor="red", label="val set, no drop")
xticks = np.array([i+1 for i in range(len(chosen_vals))])
labels = np.array([i*(val_every_n_iters + int((1/batch_size)*val_every_n_iters)) for  i in range(len(val_accs_all_iters))])
#plt.scatter(xticks, chosen_vals_full, marker="o", markersize=10, markerfacecolor="red", markeredgecolor="red", label="full network")
full_label = "full network"
plt.scatter(xticks, chosen_vals_full, marker="o", color="red", label=full_label)
plt.xticks(xticks, labels[::label_every_n])
plt.title("Accuracy distribution over coarsened networks")
plt.xlabel("work units")
plt.ylabel("accuracy")
#plt.show()
legend_labels.append((mpatches.Patch(color="red"), full_label))
plt.legend(*zip(*legend_labels))
plt.savefig(os.path.join(plots_savepath, "coarsen_batch" + str(batch_size) + "_numhidden" + str(num_hidden_nodes)), dpi=300, bbox_inches="tight")
plt.clf()
