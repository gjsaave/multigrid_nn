...
for batch_size in [1, 64, 128, 5000]:
    ...
    exp_folder = "seed" + str(random_seed) + "_batchsize" + str(batch_size) + "_droprate" + "_momentum" + str(momentum) + "_weightdecay" + str(weight_decay) + "_numlayers" + str(num_layers) + "_lr" + str(base_lr)
    ...
    for epoch in range(epochs):
        _acc, _loss, model = train_mgdrop(train_loader, model, criterion, optimizer, batch_size, mgdrop, levels, base_lr, momentum, weight_decay)
        train_accs.append(_acc)
        train_losses.append(_loss.item())
        print("epoch: ", epoch)
        print("train acc: ", _acc)
        
        _acc, _loss = validation_mgdrop(val_loader, model, criterion, batch_size)
