...
from train_test_methods import train_and_val, train_and_val_mgdrop, train_and_val_smgdrop
...
parser.add_argument('--vx_term', type=str2bool, default=True, help='true to use tau correction in coarse grid, false for no tau correction')
parser.add_argument('--same_drop_all_levels_fc', type=str2bool, default=False, help="if we want the same drop_rate for all levels e.g. FC drop_rate=0. This takes the drop_rate for that type of layer and applies it al levels of mgdrop.")
parser.add_argument('--same_drop_all_levels_conv', type=str2bool, default=False, help="if we want the same drop_rate for all levels e.g. FC drop_rate=0. This takes the drop_rate for that type of layer and applies it al levels of mgdrop.")
parser.add_argument('--N_c', type=int, default=1, help="number of coarse optimzation iterations")
parser.add_argument('--data_subset', type=float, default=1.0, help="the fraction of the total data to use for training. 1 is the whole dataset. only works for peaks right now")
...
vx_term = args.vx_term
same_drop_all_levels_fc = args.same_drop_all_levels_fc
same_drop_all_levels_conv = args.same_drop_all_levels_conv
N_c = args.N_c
...
if alg == "baseline":
    exp_folder =  "sd" + str(random_seed) + "_bs" + str(batch_size) + "_mm" + str(momentum) + "_wd" + str(weight_decay) + "_nl" + str(num_layers) + "_lr" + str(base_lr) + "_ne" + str(num_evals) + "_dr" + str(drop_rate)  + "_mt" + str(model_type) + "_nh" + str(num_hidden_nodes) + "_vn" + str(val_every_n_iters)  + "_om" + str(opt_method) + "_ds" + str(dataset) + "_nc" + str(num_layers_conv) + "_ic" + str(in_channels) + "_oc" + str(out_channels_init) + "_ks" + str(kernel_size) + "_lm" + str(loss_method) + "_pd" + str(padding) + "_drc" + str(drop_rate_conv) + "_ds" + str(data_subset)
elif alg == "mgdrop" or alg == "smgdrop":
    exp_folder = "sd" + str(random_seed) + "_bs" + str(batch_size) + "_mm" + str(momentum) + "_wd" + str(weight_decay) + "_nl" + str(num_layers) + "_lr" + str(base_lr) + "_dr" + str(drop_rate) + "_lv" + str(levels) + "_mt" + str(model_type) + "_ch" + str(compute_hessian) + "_cg" + str(clear_grads) + "_sw" + str(store_weights) + "_bls" + str(bls_num_test_points)  + "_nh" + str(num_hidden_nodes)  + "_vn" + str(val_every_n_iters) + "_om" + str(opt_method) + "_ds" + str(dataset) + "_nc" + str(num_layers_conv) + "_ic" + str(in_channels) + "_oc" + str(out_channels_init) + "_ks" + str(kernel_size) + "_lm" + str(loss_method)  + "_pd" + str(padding)  + "_drc" + str(drop_rate_conv) + "_vxt" + str(vx_term) + "_sdf" + str(same_drop_all_levels_fc) + "_sdc" + str(same_drop_all_levels_conv) + "N_c" + str(N_c) + "_ds" + str(data_subset)

if alg == "smgdrop":
    batch_size = args.batch_size*(2*levels + 1)

if dataset == "peaks":
    train_data = PeaksData(train_or_val="train", data_path=data_path)
    if data_subset < 1.0:
        sub_indices = np.random.randint(0, len(train_data), size=int(len(train_data)*data_subset))
        train_data = torch.utils.data.Subset(train_data, sub_indices)
    train_loader = DataLoader(train_data, shuffle=True, batch_size=batch_size)
    val_data = PeaksData(train_or_val="val", data_path=data_path)
    val_loader = DataLoader(val_data, shuffle=True, batch_size=batch_size)

elif dataset == "mnist":
    train_loader, val_loader = get_mnist_data(data_path, batch_size, args.debug, data_subset)
    im_size = 28
...
mgdrop = None
if alg == "mgdrop" or alg == "smgdrop":
    mgdrop = MGDrop(total_num_layers, levels, alpha, compute_hessian, clear_grads, store_weights, bls_num_test_points, dataset=dataset, opt_method=opt_method, loss_method=loss_method, model_type=model_type, drop_rate=drop_rate, drop_rate_conv=drop_rate_conv, device=device, vx_term=vx_term, same_drop_all_levels_fc=same_drop_all_levels_fc, same_drop_all_levels_conv=same_drop_all_levels_conv)
...
for epoch in range(epochs):
    if alg == "baseline":
        train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model, full_idx, val_iters_list = train_and_val(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, full_idx, num_evals, dataset, device, val_iters_list)

    elif alg == "mgdrop":
         train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model, full_idx, val_iters_list = train_and_val_mgdrop(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, mgdrop, levels, base_lr, momentum, weight_decay, full_idx, dataset, device, N_c, val_iters_list)

    elif alg == "smgdrop":
         train_acc, train_loss, val_acc, val_loss, train_accs_all_iters, val_accs_all_iters, model, full_idx, val_iters_list = train_and_val_smgdrop(train_loader, val_loader, model, criterion, optimizer, batch_size, train_accs_all_iters, val_accs_all_iters, val_every_n_iters, mgdrop, levels, base_lr, momentum, weight_decay, full_idx, dataset, device, N_c, val_iters_list)
